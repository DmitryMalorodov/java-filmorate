...
@DisplayName("Проверка удаления дизлайка с отзыва")
public class DeleteDislikeFromReviewTests extends ReviewTest {

    @Autowired
    private ReviewReactionRepository reviewReactionRepository;

    @Autowired
    public DeleteDislikeFromReviewTests(ReviewRepository reviewRepository) {
        super(reviewRepository);
    }

    @Test
    void checkDeleteDislikeFromReview() throws Exception {
        ...
        mockMvc.perform(delete(REVIEWS_ID_DISLIKE, reviewId, userId))
                .andExpect(status().isOk());

        Assertions.assertTrue(
                reviewReactionRepository.getReactionType(reviewId, userId).isEmpty(),
                "Дизлайк должен быть полностью удалён"
        );
    }

    @Test
    void checkDeleteDislikeFromReviewWithDoesNotExistReview() throws Exception {
        Long userId = getIdFromObject(createUser(user));
        Long notExistReviewId = 999L;

        mockMvc.perform(delete(REVIEWS_ID_DISLIKE, notExistReviewId, userId))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").value(String.format(REVIEW_NOT_FOUND_MESSAGE, notExistReviewId)));
    }

    @Test
    void checkDeleteDislikeFromReviewDB() throws Exception {
        Long userId = getIdFromObject(createUser(user));
        Long filmId = getIdFromObject(createFilm(film));
